...
const createFilterFunction = (completed: boolean): (() => Todo[]) => {
  return (): Todo[] => _todo_list.value.filter(
    (item: Todo) => item.checked == completed)
}
...
const
  ...
  _done: ComputedRef<Todo[]> = computed(createFilterFunction(true))
